...
app.get('/api/devices/token/:deviceId', (req, res) => {
...
app.post("/api/devices/generate-qr", async (req, res) => {
  try {
    ...
    // Create JWT token and store it with device but don't put it in the QR
    const token = jwt.sign({ deviceId }, process.env.JWT_SECRET, {
      expiresIn: "30d",
    });

    // Store device with token in memory
    ...
    // QR data: just encode deviceId (short string)
    const qrCodeUrl = await QRCode.toDataURL(deviceId);
...
app.post("/api/devices/enroll", async (req, res) => {
  try {
    ...
    if (!deviceId || !token) {
      return res
        .status(400)
        .json({ error: "Device ID and token are required" });
    }
    const device = devices.get(deviceId);
...
app.post("/api/customers/add", async (req, res) => {
  try {
    ...
    if (!name || !email || !emiPerMonth || !downpayment || !deviceId) {
      return res
        .status(400)
        .json({ error: "All customer details are required" });
    }
    const device = devices.get(deviceId);
    if (!device) {
      return res.status(404).json({ error: "Device not found" });
    }
    const customerId = `customer-${Math.random()
      .toString(36)
      .substring(2, 15)}`;
    customers.set(customerId, {
      name,
      email,
      emiPerMonth: Number(emiPerMonth),
